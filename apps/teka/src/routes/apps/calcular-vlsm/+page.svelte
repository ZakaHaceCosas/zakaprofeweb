...
{#each ranges as range, index (index)}
    <div class="mb-3 flex w-full flex-row items-center gap-3">
        ...
        <Button title="Eliminar este rango." tail="flex-1!" onclick={() => deleteRange(index)}
            >Eliminar</Button
        >
    </div>
{/each}
...
<div style="display: flex; flex-direction: row; gap: 10px; width: 100%;">
    <Button onclick={addRange} title="Agregar un nuevo rango a la lista."
        ><b>+</b> Agregar Rango</Button
    >
    <Button onclick={calculateVLSM} title="Calcular el VLSM."><b>&starf;</b> Calcular VLSM</Button>
    <Button
        onclick={share}
