...
<p>
    ...
    actualizada con la legislación vigente y de tomar en cuenta todas las variables posibles para la
    mayor precisión.<br />Contacte conmigo para reportar errores, contenido desactualizado y demás.
</p>
<hr />
...
<div style="display: flex; flex-direction: row; gap: 10px; width: 100%;">
    <Button onclick={calculateLiquidation} title="Calcular el finiquito con los datos introducidos."
...
{#if resultado}
    ...
    <p>
        ...
        <span class="font-serif">{resultado.data.dailyWage.toFixed(2)} €</span>. Este valor sale de
        sumar tu base anual (la mensual que nos diste, por doce) y tus dos pagas extra, dividido
        entre los {yearSize()} días del año.
    </p>
    <br />
    <h3>Salario que te queda por cobrar del mes</h3>
    <p>
        Te corresponden <span class="font-serif">{resultado.result.pendingWage.toFixed(2)} €</span>
        por los
        <span class="font-serif">{resultado.data.daysSinceMonth}</span> días trabajados el mes del finiquito.
    </p>
    <br />
    <h3>Pagas extraordinarias</h3>
    <p>
        Has acumulado <span class="font-serif">{resultado.result.extraPayouts.toFixed(2)} €</span>
    ...
    <p>
        Por los <span class="font-serif">{resultado.data.daysSinceYear}</span> días que llevas de
        año, y restando los
    ...
    {#if resultado.result.ind > 0 || resultado.result.lateness > 0}
        ...
        {#if resultado.result.lateness > 0}
            <p>
                Te han avisado tarde, cada día sin avisar se paga. Te dan <span class="font-serif"
                    >{resultado.result.lateness.toFixed(2)} €</span
                >.
            </p>
        {/if}
    {/if}
