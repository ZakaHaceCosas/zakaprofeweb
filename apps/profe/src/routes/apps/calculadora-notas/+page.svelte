...
{#each notas as nota, index (index)}
    <div class="mb-3 flex w-full flex-col items-center gap-3 md:flex-row">
        ...
        <Button
            ...
            onclick={() => deleteNota(index)}>Eliminar</Button
        >
    </div>
{/each}
<br />
...
<div style="display: flex; flex-direction: row; gap: 10px; width: 100%;">
    <Button onclick={addNota} title="Añadir una nota más con la que mediar."
        ><b>+</b> Agregar Nota</Button
    >
    <Button onclick={calculateAverage} title="Calcular la media de las notas que has introducido."
...
{#if !isPonderingOne && notas.filter((n) => !isNaN(parseFloat(n.pondering))).length != 0 && notas
    ...
    <br />
    <p>¡Ojo! El valor podría no ser lógico, mira que las ponderaciones sumen 100 (no lo hacen).</p>
{/if}
