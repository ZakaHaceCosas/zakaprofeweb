<script lang="ts">
    ...
    onMount(() => {
        ...
        if (expr !== "" && op !== null && nVal !== null && iVal !== null) calculate();
    });
...
<main>
    ...
    <div class="flex w-full flex-row gap-2">
        <select
            ...
            bind:value={op}
        >
            <option value="null" selected disabled>Elige un operador agregatorio...</option>
            <option value="∑">Sumatorio (∑)</option>
            <option value="∏">Productorio (∏)</option>
        </select>
        <Input
            type="text"
            name="n-val"
    ...
    <div style="display: flex; flex-direction: row; gap: 10px; width: 100%;">
        <Button onclick={calculate} channel="ZakaTeka" title="Calcular la IP."
